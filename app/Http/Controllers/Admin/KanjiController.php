...
class KanjiController extends Controller
{
    ...
    protected function formPage(Kanji $kanji, string $mode, string $action)
    {
        return view('vue-page', [
            ...
            'pageProps' => [
                ...
                'existingKanji' => Kanji::query()
                    ->orderBy('sort_order')
                    ->get(['id', 'jlpt_level_id', 'source_id', 'chapter', 'slug', 'sort_order'])
                    ->map(fn (Kanji $item) => [
                        'id' => $item->id,
                        'jlpt_level_id' => $item->jlpt_level_id,
    ...
    {
        $this->mergeAutoGeneratedFields($request, $kanji);
        ...
        $validated = $request->validate([
            ...
            'sort_order' => [
                ...
                'min:1',
                'max:9999',
                function (string $attribute, mixed $value, \Closure $fail) use ($request, $kanji) {
                    $query = Kanji::query()
                        ->where('jlpt_level_id', $request->integer('jlpt_level_id'));
                    ...
                    if ($chapter === null) {
                        $query->whereNull('chapter');
                    } else {
                        $query->where('chapter', $chapter);
                    }

                    if ($kanji) {
                        $query->whereKeyNot($kanji->id);
                    }

                    if ($query->where('sort_order', (int) $value)->exists()) {
                        $fail('This sort order is already used for the selected JLPT level, category, and chapter.');
                    }
                },
            ],
    ...
    protected function mergeAutoGeneratedFields(Request $request, ?Kanji $kanji = null): void
    {
        ...
        $request->merge([
            'slug' => $slug !== ''
                ...
                : $this->generateUniqueSlug(
                    $request->string('meaning')->toString(),
                    $request->string('character')->toString(),
                    $kanji?->id,
                ),
            'sort_order' => filled($sortOrder)
                ? $sortOrder
                : $this->nextSortOrderForScope(
                    $request->integer('jlpt_level_id') ?: null,
    ...
    protected function generateUniqueSlug(string $meaning, string $character, ?int $ignoreId = null): string
    {
        $base = Str::slug($meaning);

        if ($base === '') {
            $base = Str::slug($character);
        }
    ...
    protected function nextSortOrderForScope(?int $levelId, ?int $sourceId, mixed $chapter, ?int $ignoreId = null): int
    {
        if ($levelId === null) {
            return 1;
        }

        $query = Kanji::query()->where('jlpt_level_id', $levelId);
