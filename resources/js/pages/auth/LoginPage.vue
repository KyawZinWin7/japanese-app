<template>
    <main class="auth-shell">
        <section class="auth-card">
            ...
            <p class="app-subtitle">{{ t('auth.loginText') }}</p>
            ...
            <form :action="routes.login" method="POST" class="mt-6 space-y-5">
                ...
                <div>
                    ...
                    <input id="password" name="password" type="password" class="app-input" autocomplete="current-password" required>
                    <p v-if="errors.password?.length" class="app-help">{{ errors.password[0] }}</p>
                </div>

                <button type="submit" class="app-btn w-full">{{ t('auth.loginTitle') }}</button>
            </form>

            <p class="mt-6 text-sm text-slate-600">
                {{ t('auth.noAccount') }}
                <a :href="routes.register" class="font-semibold text-emerald-700 hover:text-emerald-600">{{ t('auth.createOne') }}</a>
            </p>
        </section>
    </main>
</template>
