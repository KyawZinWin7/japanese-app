        ...
        <main class="page-shell">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <h1 class="app-title">Manage Exam Practice</h1>
                    <p class="app-subtitle">Create separate practice sets without touching your existing study modules.</p>
                </div>
                <div class="flex items-center gap-3">
                    <a :href="routes.publicIndex" class="app-link">View public list</a>
                    <a :href="routes.create" class="app-btn">New Set</a>
                </div>
            </div>
            ...
            <div v-if="pagination.total" class="mt-8 flex flex-wrap items-center justify-between gap-3">
                <p class="text-sm text-slate-500">Showing {{ pagination.from }}-{{ pagination.to }} of {{ pagination.total }} sets</p>
            ...
            <div class="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Set</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Exam Code</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        <tr v-for="item in items" :key="item.slug">
                            ...
                            <td class="px-4 py-4 text-slate-600">{{ item.exam_code || '-' }}</td>
            ...
            <p v-if="!items.length" class="app-empty mt-8">No exam practice sets found.</p>
            <div v-if="pagination.total" class="mt-8 text-sm text-slate-500">Showing {{ pagination.from }}-{{ pagination.to }} of {{ pagination.total }} sets</div>
            <PaginationNav :pagination="pagination" />
        </main>
    </AdminLayout>
</template>
