                    ...
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td class="px-4 py-4">
                            <div class="flex items-center justify-end gap-3">
                                ...
                                <form :action="`${routes.editBase}/${item.slug}`" method="POST" @submit="confirmDelete($event, item.word)">
...
async function confirmDelete(event, word) {
    event.preventDefault();

    const result = await Swal.fire({
        title: 'Are you sure?',
        html: `<div class="swal-delete-copy">Delete <strong>${word}</strong>?</div>`,
        icon: 'warning',
