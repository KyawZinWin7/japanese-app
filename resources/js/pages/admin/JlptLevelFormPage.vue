<template>
    <AdminLayout
        :actions="layout.actions"
        :csrf-token="csrfToken"
        :navigation="layout.navigation"
        :subtitle="layout.subtitle"
        :title="layout.title"
        :user="layout.user"
    >
    <main class="mx-auto max-w-3xl">
        <div class="mb-6">
            <p class="text-sm font-semibold uppercase tracking-[0.25em] text-emerald-600">Admin</p>
            <h1 class="mt-3 text-3xl font-semibold text-slate-900">
                {{ mode === 'create' ? 'Create JLPT Level' : 'Edit JLPT Level' }}
            </h1>
            <p class="mt-2 text-slate-600">Manage JLPT level details with a simple Vue form.</p>
        </div>

        <JlptLevelForm
            :action="routes.action"
            :csrf-token="csrfToken"
            :errors="errors"
            :index-url="routes.index"
            :level="level"
            :method="method"
            :submit-label="mode === 'create' ? 'Create Level' : 'Update Level'"
        />
    </main>
    </AdminLayout>
</template>

<script setup>
import AdminLayout from '../../components/admin/AdminLayout.vue';
import JlptLevelForm from '../../components/jlpt/JlptLevelForm.vue';

defineProps({
    csrfToken: { type: String, required: true },
    errors: { type: Object, required: true },
    layout: { type: Object, required: true },
    level: { type: Object, required: true },
    method: { type: String, required: true },
    mode: { type: String, required: true },
    routes: { type: Object, required: true },
});
</script>
